...
private:
    ...
    void cloud_callback(const sensor_msgs::msg::PointCloud2::SharedPtr msg) {
        ...
        try {
            tf_buffer_->transform(*msg, transformed_cloud, "map");
        } catch (const tf2::TransformException &ex) {
            RCLCPP_ERROR(this->get_logger(), "Could not transform pointcloud to map: %s", ex.what());
            return;
        }
    ...
    // Midpoint Line Drawing algorithm but for tracing line from car to point cloud
    ...
    void trace_line_along_y(int x1, int y1, int x2, int y2, std::unordered_set<int>& occupied_cells){
        ...
        int x = x1;
        for (int y = y1; y < y2; y++) {
            if(handle_cell(x, y, occupied_cells)){
                break;
            }
            if (f < 0) {
                x += change;
        ...
        grid.data[y2 * grid.info.width + x2] = 100;
    }
    ...
    bool handle_cell(int x, int y, std::unordered_set<int>& occupied_cells){
